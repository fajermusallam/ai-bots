...
// ══════════════════════════════════════════════════════
// ✅ HTML FILE GENERATOR — بدلاً من Word (عربية مثالية)
// ══════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════
// ✅ استدعاء AI منفصل لتوليد محتوى الملف
// المشكلة القديمة: كان يأخذ رد المحادثة كملف
// الحل: يطلب محتوى وثيقة مستقل ومنسق
// ══════════════════════════════════════════════════════
  ...
  try {
    ...
    if (needsFile(message)) {
      try {
        ...
        // ✅ استدعاء AI منفصل — محتوى الوثيقة مستقل عن رد المحادثة
        const fileContent = await generateFileContent(message, PROMPTS[botId]);

        const title = message
          .replace(/[^\u0600-\u06FF\u0750-\u077F\s\w]/g, '')
          .trim().slice(0, 50) || 'وثيقة شاملة';
